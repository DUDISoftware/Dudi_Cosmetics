...
const TrendingProducts = () => {
  const [visibleProducts, setVisibleProducts] = useState(products.slice(0, 4)); // Show first 4 products
  ...
  const scroll = (direction) => {
    if (direction === "right") {
      ...
      setVisibleProducts((prevProducts) => [
        ...
        products[(products.indexOf(prevProducts[prevProducts.length - 1]) + 1) % products.length],
      ]);
    } else if (direction === "left") {
      // Move the last product to the front
      setVisibleProducts((prevProducts) => [
        products[(products.indexOf(prevProducts[0]) - 1 + products.length) % products.length],
